...
$(function () {
  ...
  if (dt_scrollable_table.length) {
    var dt_scrollableTable = dt_scrollable_table.DataTable({
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              '<a href="javascript:;" class="btn btn-sm btn-text-secondary rounded-pill btn-icon dropdown-toggle hide-arrow" data-bs-toggle="dropdown" onclick="setZIndex(this)"><i class="ti ti-dots-vertical ti-md"></i></a>' +
      ...
      fixedColumns: {
        leftColumns: 0,  
        rightColumns: 1  // Fija las dos primeras columnas de la izquierda
    },
  ...
  document.getElementById('addRecordForm').addEventListener('submit', function(event) {
    ...
    // Validar el formulario (esto ya lo hace el atributo 'required')
    if (this.checkValidity()) {
      // Obtener el máximo id actual
      const maxId = dt_scrollableTable.rows().data().reduce((max, row) => {
      ...
      dt_scrollableTable.row.add(newRecord).draw(); // Usa DataTables para agregar la fila

      // Cerrar el offcanvas manualmente
      ...
      document.getElementById('addRecordForm').reset();
        ...
        this.reportValidity(); // Esto mostrará los mensajes de error de validación
    }
...
function setZIndex(element) { 
  // Selecciona todos los elementos con la clase .dtfc-fixed-right
  const allParents = document.querySelectorAll('.dtfc-fixed-right');
  
  // Restablece el z-index de todos los elementos .dtfc-fixed-right a auto
  allParents.forEach(parent => {
    parent.style.zIndex = 'auto';
  });

  // Selecciona el contenedor padre inmediato del botón que fue clicado
  const parent = element.closest('.dtfc-fixed-right');
  
  // Asigna un z-index alto al contenedor padre que fue clicado
  parent.style.zIndex = 999;

  // (Opcional) Puedes restablecer el z-index una vez que el menú se cierre si es necesario
